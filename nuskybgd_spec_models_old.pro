...
if not keyword_set(expfactor) then expfactor=1.
...
if not keyword_set(specname) then specname=specdir+ab+'_sr.pha'
...
; Non-focused CXB
;printf, lun, 'da '+data_ind+' '+specname
...
; Internal background
printf, lun, 'response '+spec_ind+':'+data_ind+' '+respfile
printf,lun,'model '+spec_ind+':intbgd'+ab+'_'+data_ind+' nuabs*(',format='($,A)'
...
if grxe then begin
   ...
   printf, lun, 'response '+spec_ind+':'+data_ind+' '+respfile
   printf, lun, 'arf '+spec_ind+':'+data_ind+' '+bearf
   printf,lun,'model '+spec_ind+':grxe'+ab+'_'+data_ind+' nuabs*(gauss+atable{'+auxildir+'polarmodel.fits})'
   printf,lun,str(spt)+' -1'
   printf,lun,str(sczt)+' -1'
   printf,lun,'0. -1'
   printf,lun,'0.9 -1'
...
print,'Background model xcm files created:'
print,outdir+'/'+file_basename(specname, '.pha')+'_bgd_models.xcm
